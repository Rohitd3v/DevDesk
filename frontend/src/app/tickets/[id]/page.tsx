...
import { GitHubService } from "@/app/features/github/services/githubService";
...
function TicketDetailContent() {
  ...
  const [syncStatus, setSyncStatus] = useState<{ synced: boolean; sync_data?: unknown } | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchSyncStatus = async () => {
      try {
        const status = await GitHubService.getTicketSyncStatus(ticketId);
      ...
      <div className="bg-white rounded-xl border p-6 shadow-sm mb-6">
        <div className="flex flex-col md:flex-row md:items-start justify-between mb-4 gap-4">
          ...
          <div className="flex gap-2 items-center flex-wrap">
            ...
            {syncStatus?.synced && (syncStatus.sync_data as unknown as SyncData)?.github_linked_repos?.html_url && (
              <a
                href={`${(syncStatus.sync_data as unknown as SyncData).github_linked_repos!.html_url}/issues/${(syncStatus.sync_data as unknown as SyncData).github_issue_number}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1 text-sm px-3 py-1 rounded-full bg-gray-900 text-white hover:bg-gray-700 transition"
              ...
              >
                ...
                #{(syncStatus.sync_data as unknown as SyncData).github_issue_number}
              </a>
            )}
