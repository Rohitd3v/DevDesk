...
export const getTicketSyncStatus = async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  ...
  try {
    const { data: syncData, error } = await GitHubSyncService.getSyncByTicketId(ticketId);

    if (error || !syncData) {
      if (error && error.code !== 'PGRST116') { // PGRST116 is "The result contains 0 rows"
        throw error;
      }
      return sendResponse(res, 200, true, { synced: false });
    }
    ...
    console.error("Error fetching ticket sync status:", error);
    return sendResponse(res, 500, false, { error: error.message || "Failed to fetch ticket sync status" });
  }
};